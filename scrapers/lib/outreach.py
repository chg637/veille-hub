...
def email_draft_rncp_nouveau(compte: str, signal_text: str, url_source: str, contact_nom: str | None = None, contact_fonction: str | None = None) -> dict:
    """Email post-nouvelle fiche RNCP/RS — pitch 100% ITS (plateforme de passation)."""
    sig_short = _shorten_signal(signal_text, 120)
    sig_key = _subject_keyword(signal_text, 6)
    ...
    body = (
        ...
        f"J'ai noté votre actualité : « {sig_short} ».\n"
        f"\n"
        f"Sur une nouvelle fiche RNCP/RS, deux sujets pèsent vite côté "
        ...
        f"sur laquelle vous déposez VOS propres épreuves (théoriques, "
        f"pratiques, comportementales) et qui prend en charge :\n"
        f"\n"
        f"- Banque de questions structurée par bloc de compétences "
        f"(alignée sur votre fiche RNCP)\n"
        f"- Surveillance à distance (proctoring) avec preuves opposables\n"
        f"- Correction automatique + rapports candidat/admin\n"
        f"- Traçabilité complète des sessions, exportable pour audit\n"
        f"- Génération automatique des attestations et duplicatas\n"
        f"\n"
        f"Avantage : vous gardez la main pédagogique sur le contenu et "
        f"vous déléguez l'infrastructure de passation. Déploiement "
        f"4-6 semaines pour ouvrir la 1re session.\n"
        f"\n"
        f"20 minutes pour vous montrer comment ça tourne chez un "
        f"certificateur de taille comparable ?\n"
        f"\n"
        f"Charles GOSSET\n"
        f"Sales Manager — Isograd / ITS\n"
        f"\n"
...
def email_draft_ao(compte: str, titre_ao: str, deadline: str, url_dce: str) -> dict:
    """Email préparation réponse AO — pitch 100% ITS (plateforme SaaS d'hébergement d'examens)."""
    subject = f"AO {compte} — ISOGRAD souhaite candidater"
    body = (
        f"Bonjour,\n"
        f"\n"
        f"Nous avons identifié votre consultation :\n"
        f"\"{titre_ao}\"\n"
        f"Deadline : {deadline}\n"
        f"\n"
        f"Isograd opère ITS, une plateforme SaaS d'hébergement d'examens et "
        f"d'évaluation des compétences. C'est un moteur générique : vous y "
        f"déposez vos propres contenus (tests, examens, QCM, mises en "
        f"situation), et ITS s'occupe de la passation, de la correction "
        f"automatique, de la production des rapports et de l'intégration "
        f"avec votre SI.\n"
        f"\n"
        f"Cas d'usage couverts : tests de recrutement, examens "
        f"académiques, contrôles continus, certifications internes, "
        f"évaluations comportementales. 60+ établissements déjà déployés "
        f"(universités, grandes écoles, CFA, grands comptes publics et "
        f"privés).\n"
        f"\n"
        f"Avant la finalisation de notre dossier, nous aurions besoin de "
        f"quelques clarifications :\n"
        f"- Volume cible de candidats / sessions par an\n"
        f"- Modalités de surveillance attendues (proctoring, télésurveillance)\n"
        f"- Périmètre fonctionnel (banque de questions, génération "
        f"d'attestations, exports, intégration SI existant)\n"
        f"- Calendrier prévisionnel de mise en œuvre\n"
        f"\n"
        f"Pouvons-nous organiser un échange de 30 minutes ?\n"
        f"\n"
        f"Bien cordialement,\n"
        f"Charles GOSSET\n"
        f"Sales Manager — Isograd / ITS\n"
